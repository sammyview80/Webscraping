...
def find_title():
    locator = 'head title'
    item_title = soup.select_one(locator).string
    item_list = [item for item in item_title]
    item_title = item_list[:-28]
    item_title = ''.join(item_title)
    return item_title


def quations():
    locator = 'body.default header.header div.page_inner div.row small'
    item_quations = soup.select_one(locator).string
    return item_quations.replace('!','')
    

quations()

print(f"The title of the book is {find_title()} and the quations of the book is {quations()}")